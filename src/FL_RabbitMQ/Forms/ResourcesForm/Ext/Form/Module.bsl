...
Procedure SaveAndClose(Command)
    ...
    If IsBlankString(ExchangeName) Then
        FL_CommonUseClientServer.NotifyUser(NStr("en=""Field 'Exchange name' must be filled."";ru=""Поле 'Имя обмена' должно быть заполнено."";en_CA=""Field 'Exchange name' must be filled."""), , 
            "ExchangeName");
        Return;    
    EndIf;
    
    If IsBlankString(RoutingKey) Then
        FL_CommonUseClientServer.NotifyUser(NStr("en=""Field 'Routing key' must be filled."";ru=""Поле 'Ключ маршрутизации' должно быть заполнено."";en_CA=""Field 'Routing key' must be filled."""), , 
            "RoutingKey");
        Return;  
    EndIf;
